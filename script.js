...
function getRandomBrickColorPalette() {
  return brickColorPalettes[
    Math.floor(Math.random() * brickColorPalettes.length)
  ];
}
...
function resizeCanvas() {
  const minWidth = 320;
  const maxWidth = 800;
  const idealHeightRatio = 1.3; // Canvas é 1.3x mais alto que largo

  let newWidth = window.innerWidth * 0.9;
  newWidth = Math.max(minWidth, Math.min(newWidth, maxWidth));

  // 1. Calcula a altura ideal baseada na largura (conforme sua proporção)
  let newHeight = newWidth * idealHeightRatio;

  // 2. Limita a altura máxima pela altura da viewport (90% para deixar espaço para controles)
  const maxHeightAvailable = window.innerHeight * 0.9;

  // 3. Verifica qual dimensão deve ser a principal:
  if (newHeight > maxHeightAvailable) {
    // Se a altura ideal for maior que a altura disponível (caso widescreen)
    newHeight = maxHeightAvailable;
    // Recalcula a largura para manter a proporção (newWidth = newHeight / ratio)
    newWidth = newHeight / idealHeightRatio;
  }

  // 4. Garante que a largura respeite o limite máximo e mínimo
  newWidth = Math.max(minWidth, Math.min(newWidth, maxWidth));
  newHeight = newWidth * idealHeightRatio; // Recalcula altura final
...
function createBricks(levelData) {
  ...
  const totalHorizontalPadding =
    brick.offsetLeft * 2 + (colCount - 1) * brick.padding;
...
function drawBall(b) {
  ...
  const gradient = ctx.createRadialGradient(
    b.x - b.radius / 3,
    b.y - b.radius / 3,
    b.radius / 4, // Ponto de luz
    b.x,
    b.y,
    b.radius // Centro da bola
  );
...
function moveBall(b) {
  ...
  if (
    b.y + b.radius > canvas.height - paddle.height - 10 &&
    ...
    b.dx = normalizedRelativeIntersectionX * -Math.abs(b.dy);
  }
}
    ...
    for (let r = 0; r < bricks[c].length; r++) {
      ...
      if (b && b.status > 0) {
        balls.forEach((ballInstance) => {
          if (
            ballInstance.x + ballInstance.radius > b.x &&
            ...
            if (
              prevBallY + ballInstance.radius <= b.y ||
              prevBallY - ballInstance.radius >= b.y + brick.height
            ) {
...
function activateSpecialBlock(originBall) {
  // A propriedade `originBall` já contém o dx e dy acelerados.

  // Cria a primeira bola: inverte X, mantém Y.
  const newBall1 = Object.assign({}, originBall, {
    dx: -originBall.dx,
    // Não é necessário modificar originBall.y ou originBall.dy,
    // pois Object.assign({}, originBall) já as copia.
  });

  // Cria a segunda bola: mantém X, inverte Y.
  const newBall2 = Object.assign({}, originBall, {
    dy: -originBall.dy,
  });
...
leftButton.addEventListener("touchcancel", (e) => {
  // Importante para quando o toque é interrompido
  e.preventDefault();
  paddle.movingLeft = false;
});
